#!/usr/bin/env bash
#
# AtmoFrance - Mise en place du DAG dans le dossier Airflow
#
# Cree le fichier DAG directement au bon emplacement (infra/airflow/dags/)
# avec le contenu valide, sans dependre d'un fichier telecharge.
#
set -euo pipefail

if [ ! -d infra/airflow ]; then
  echo "ERREUR : lance ce script depuis atmofrance/ (apres setup_phase10.sh)"
  exit 1
fi
...
echo ">> Ecriture du DAG dans $DAGS_DIR/atmofrance_pipeline.py..."
cat > "$DAGS_DIR/atmofrance_pipeline.py" << 'PYEOF'
...
# Fichier de la veille (donnees consolidees a J+1).
...
# Verification : le DAG se parse-t-il avec l'Airflow installe ?
echo ">> Validation du DAG..."
AIRFLOW_HOME="$PROJET/infra/airflow/home" ATMOFRANCE_HOME="$PROJET" \
  "$PROJET/.venv-airflow/bin/python" -c "
import sys
...
echo " DAG en place."
echo "=============================================="
echo " Airflow le detectera automatiquement sous 5 min, ou immediatement"
echo " si tu redemarres le lanceur :"
echo "   1) dans le terminal Airflow : Ctrl+C"
echo "   2) bash infra/airflow/lancer_airflow.sh"
echo ""
echo " Puis rafraichis http://localhost:8080 : le DAG 'atmofrance_pipeline'"
echo " apparaitra dans la liste."
echo "=============================================="
